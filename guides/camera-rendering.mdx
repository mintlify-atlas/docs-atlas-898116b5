...
cam.Depth         = 0;        // Lower values render first
cam.HDR           = true;
```
...
<CardGroup cols={2}>
  <Card title="FieldOfView" icon="eye">
    Vertical field of view in degrees (perspective mode only). Typical values range from 50° to 90°. Use `Camera.IsOrthographic` to check the active mode.
  </Card>
  ...
  <Card title="Depth" icon="layer-group">
    Render order among multiple cameras. `SceneManager.Draw()` sorts cameras ascending by `Depth`. Cameras with lower `Depth` render first (i.e., behind higher-depth cameras).
  </Card>
  ...
  <Card title="HDR" icon="sun">
    When enabled, the forward buffer uses a 16-bit float format (`R16_G16_B16_A16_Float`) instead of 8-bit (`R8_G8_B8_A8_UNorm`), enabling physically based lighting ranges and Bloom.
  </Card>
</CardGroup>
...
```csharp
// Create a render texture at 512×512 with an HDR color buffer
RenderTexture minimapRT = new RenderTexture(512, 512,
...
foreach (Camera cam in cameras)  // sorted by Depth
{
...
public class WireframeRenderPipeline : RenderPipeline
{
    public override void Render(Camera camera, in RenderingData data)
    {
        ...
        // Draw all batches using only their ShadowCaster pass as a wireframe stand-in
...
|---|---|
| `OnPreCull(Camera camera)` | Before the camera culls renderables — use it to force `DepthTextureMode` flags |
