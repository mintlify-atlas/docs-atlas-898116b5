...
`Time.frameCount` is a monotonically increasing counter incremented once per rendered frame. Use it to spread heavy work across frames with a modulo check.
...
`Time.timeScale` is a multiplier applied to `deltaTime` and `time`. The default is `1.0`. Setting it to `0` effectively pauses all scaled time; setting it to `0.25` creates quarter-speed slow motion.
...
`Time.smoothDeltaTime` is an exponentially weighted moving average of the raw frame delta, smoothed using `timeSmoothFactor` (default `0.25`). It dampens sudden spikes caused by garbage collection or asset loading, making it useful for values that should feel silky even under jitter.
...
<CardGroup cols={2}>
  ...
  <Card title="Time.frameCount" icon="list-ol">
    Frames rendered since launch. Use for frame-staggered work.
  </Card>
  <Card title="Time.timeScale" icon="gauge-high">
    Global time multiplier. `0` = paused, `0.5` = half speed, `1` = normal.
  </Card>
  <Card title="Time.smoothDeltaTime" icon="wave-sine">
    Exponentially smoothed frame delta. Use for smooth display values and spring dampers.
  </Card>
</CardGroup>
