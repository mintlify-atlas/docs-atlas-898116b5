...
A coroutine is started with `StartCoroutine(string methodName)` — passing the name of a method on the same `MonoBehaviour` that returns `IEnumerator`. The engine uses reflection to locate the method by name, invokes it, and wraps the resulting `IEnumerator` in a `Coroutine` object that advances each frame (or each fixed step, depending on the yield instruction in use).
...
<CardGroup cols={2}>
  ...
  <Card title="Starting a duplicate name throws" icon="tag">
