---
title: 'Rigid Body Physics with the Jitter2 Engine in Prowl'
...
<Tabs>
  ...
  <Tab title="Layer-masked">
    ...
    ```csharp
    // Build a mask that includes only layer 0
    LayerMask groundMask = new LayerMask();
    groundMask.SetLayer(0);

    if (Physics.Raycast(transform.position, Vector3.down,
                        out RaycastHit hit, 1.1, groundMask))
    {
        isGrounded = true;
    }
    ```
