---
title: 'Navigation Mesh and AI Agents in Prowl'
...
<Tabs>
  <Tab title="Static Rigidbodies (default)">
    ...
    surface.GeometryLayers = LayerMask.FromLayer(0); // layer 0 only
    ```
  </Tab>
...
agent.height               = 1.8f;   // Agent height for path clearance
...
Use `NavMeshSurface.MoveToTarget` to send a specific agent to a world-space position:

```csharp
NavMeshSurface surface = agent.Surface;
Vector3 worldDestination = player.Transform.position;

// Convert to the surface's local space, then request move
RcVec3f localPos = new RcVec3f(
    (float)worldDestination.x,
    (float)worldDestination.y,
    (float)worldDestination.z);

surface.MoveToTarget(agent, localPos, adjust: false);
```
...
public class EnemyChase : MonoBehaviour
{
    ...
    {
        // Positions are stored in NavMeshSurface local space
