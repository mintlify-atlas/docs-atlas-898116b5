...
Tears down the active scene: fires `[OnSceneUnload]` attribute callbacks, destroys the current `Scene` immediately (which in turn destroys all `GameObject`s and components), processes the engine's destroy queue via `EngineObject.HandleDestroyed()`, resets the main camera reference, and replaces `Current` with a fresh empty `Scene`.
