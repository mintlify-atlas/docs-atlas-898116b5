...
<Tabs>
  ...
  <Tab title="From Prefab">
    ```csharp
    // In play mode: deep-copies the prefab data
...
Re-parents this `GameObject` under `NewParent`. Optionally preserves the object's world-space position and rotation.
...
Returns `true` if this object is anywhere in the descendant hierarchy of `parent`.
...
Gets or sets the zero-based index of this object within its parent's `children` list.
...
Searches this object and optionally all descendants for a child with a matching `Guid` identifier.
...
Finds a component by its persistent `Guid` identifier rather than type.
...
Deep-clones an existing `GameObject` and adds it to the active scene.
...
<ResponseField name="returns" type="GameObject">
  The cloned `GameObject` already added to the active scene.
</ResponseField>
...
Instantiates from an `AssetRef<Prefab>`. Returns `null` if the reference is unavailable.
