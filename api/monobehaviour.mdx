...
description: 'Full reference for Prowl's MonoBehaviour: all lifecycle methods, coroutine API, component accessors, and callback signatures.'
---
...
<ResponseField name="ExecuteAlways" type="bool">
  When `true` (set by `[ExecuteAlways]` attribute), lifecycle methods fire even when the game is not in play mode. Configurable at runtime but normally controlled by the attribute.
...
Override any of the following `virtual` methods in your derived class. You do **not** call the base implementation unless you explicitly need to chain behavior.
...
Called on every `MonoBehaviour` in a scene immediately after that scene finishes loading via `SceneManager.LoadScene`.
...
<ResponseField name="OnRenderImage" type="virtual void">
  Called after the camera attached to this `GameObject` has rendered the scene. Implement post-processing effects here. Use `[ImageEffectOpaque]` to run before transparent geometry.
...
Stops the coroutine with the given method name. Has no effect if no coroutine with that name is running.
...
Suspends the coroutine for approximately `seconds` game-time seconds (measured against `Time.time`).
...
```csharp
yield return new WaitForSeconds(2.5f); // wait 2.5 s
```
...
Controls the order of this component within the owning `GameObject`'s component list. Components are sorted by `[ExecutionOrder]` on `AddComponent`, but you can fine-tune with sibling index.
...
These attributes modify the behavior of `OnRenderImage` callbacks.
