...
<ResponseField name="IsRunning" type="bool">
  `true` from the moment the window opens until `AppClose` fires. Set to `false` automatically at shutdown; you can read this field to detect whether the engine loop is still active.
...
<ResponseField name="IsPlaying" type="bool">
  `true` when the game simulation is running. In a standalone build this is always `true`. In the Prowl Editor it is `false` while in edit mode and `true` while the Play button is active.
...
<ResponseField name="Update" type="event Action">
  Fires once per frame **before** `Render`. The engine's internal `AppUpdate` method pushes a `TimeData` stack frame, invokes audio pooling, then raises this event. Scene update logic and input polling belong here.
...
<ResponseField name="Render" type="event Action">
  Fires once per frame **after** `Update`, still within the same `TimeData` scope. Camera rendering, post-processing, and UI drawing should subscribe here.
...
<ResponseField name="Quitting" type="event Action">
  Fires when the application is closing, before graphics and audio are disposed. Use this to flush save data, release unmanaged handles, or log shutdown telemetry.
...
Starts the full engine loop. This call **blocks** until the window is closed.
...
<ResponseField name="returns" type="GraphicsBackend">
  The first backend in the platform's preferred list (the engine currently returns index `[0]`).
</ResponseField>
