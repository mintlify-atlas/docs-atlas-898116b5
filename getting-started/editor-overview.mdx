---
title: 'Prowl Editor Layout and Navigation Guide'
sidebarTitle: 'Editor Overview'
description: 'Learn the Prowl editor layout: scene view, hierarchy, inspector, assets browser, console, game window, and docking system.'
---
...
You can drag assets from the **Assets Browser** directly onto the scene:

| Asset Type | Result |
|-----------|--------|
| `GameObject` / Prefab | Instantiates at the surface hit point under the cursor, or 10 units in front of the camera if no surface is hit |
...
Asset files selected in the Assets Browser are also shown in the Inspector, where you can view and edit their importer settings (e.g., texture compression, normal-map flag).

## Assets Browser (`AssetsBrowserWindow`)

The Assets Browser shows the contents of the currently selected folder inside your `Assets/` directory and lets you navigate the project's asset tree.

### Navigation

The Assets Tree (a companion panel called `AssetsTreeWindow`) on the left shows the folder hierarchy. Clicking a folder in the tree updates the Assets Browser to show that folder's contents. You can also use the search bar inside the Browser to find files by name across the entire current folder (including sub-folders).
...
<CardGroup cols={3}>
  ...
  <Card title="Assets Browser" icon="images">Windows → Assets Browser</Card>
